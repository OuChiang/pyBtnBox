...
class MenuLayout:
    """
    @staticmethod
    def LO_panel_menu_bar(MenuProp,PropName,layout,MenuIcon):
        split = layout.split(factor=0.70, align=True)
        if not MenuIcon :
            split.prop(MenuProp,PropName,text='')
            return None
        #LO_icon_enum(split,MenuIcon,MenuProp,PropName)
        #split.prop(MenuProp, "showMenu", text="",icon='SMALL_CAPS')
        return None
    """
    @staticmethod
...
# { Normal Menu Button }
class btnLayout:
    def __init__(self,name,text,pyPath,icon,type,tip):
        self.name = name
        self.text = text
        self.pyPath = pyPath
        self.icon = icon
...
class Editor_btnLayout:
    ...
    def LO_button_edit_box():
        def layout_func(Layout,Editor):
            ...
            box = Layout.box()
            column = box.column()
            column.prop(Editor, "btn_type",text='Type')
            column.separator(factor=2.0,type='LINE')
            lo_props(column,Editor,Type_ID)
            rowR = box.row()
        ...
        def lo_props(Layout,Editor,Type_ID):
            if Type_ID == 0: 
    ...
    def btn_active(self,Layout,btn_name,active):
        ...
        Layout.operator( "pybtnbox.editor_btn_funclist_current" ,text='',icon= "PREFERENCES"  ).button_path  = self.pyPath
